            ...
            <div id="index" class="tabcontent mt-4 mx-4 block">
                <div class="flex gap-4 mb-4">
                    <div class="relative w-1/2">
                        <input type="text"
                            class="w-full p-2 pl-8 rounded border border-gray-200 bg-gray-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <button style="margin-top: auto;margin-bottom: auto" onclick="openNewTab(event, 'create')"
                        ...
                        cupones</button>
                    <button style="margin-top: auto;margin-bottom: auto" id="button-popover"
                        class="rounded-full border-2 border-blue-500 shadow-lg h-8 w-8" aria-describedby="tooltip">?</button>
                                ...
                                @forelse ($experiences_with_coupons as $experience_coupon)
                                    <tr
                                        ...
                                        <td class="px-4 py-3 cursor-pointer">
                                            ...
                                            @endphp
                                            <p onclick='openCouponDetail(event, "show","{{ $experience_coupon->title }}",<?php echo json_encode($array_of_coupons); ?> )'
                                                class="font-semibold">{{ $experience_coupon->title }}</p>
                                        </td>
                                        ...
                                        <td class="px-4 py-3 cursor-pointer text-center">
                                            <p
                                                class="font-semibold {{ count($experience_coupon->coupon_codes->where('status', '1')) > 0 ? 'text-green-500' : '' }} ">
                                                {{ count($experience_coupon->coupon_codes->where('status', '1')) }}</p>
                                        </td>
                                        <td class="px-4 py-3 cursor-pointer text-center">
                                            <p class="font-semibold">
                                                {{ count($experience_coupon->coupon_codes->where('status', '0')) }}</p>
                                        </td>
                                        <td class="px-8 py-3 ">
                                            @if ($experience_coupon->coupon_codes->first())
                                                <i onclick="deletePlace(event,'{{ addslashes($experience_coupon->title) }}',{{ $experience_coupon->id }})"
